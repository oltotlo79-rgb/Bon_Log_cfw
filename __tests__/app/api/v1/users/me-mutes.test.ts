...
/**
 * GET /api/v1/users/me/mutes — ミュート一覧取得（カーソルページネーション）
 *
...
const mockGetMutedUsersService = vi.fn()
vi.mock('@/lib/services/mute-service', () => ({
  muteUserService: vi.fn(),
  unmuteUserService: vi.fn(),
  getMutedUsersService: (...args: unknown[]) => mockGetMutedUsersService(...args),
}))

const mockMutedUsers = [
  { id: 'muted-1', nickname: 'ミュートユーザー1', avatarUrl: '/avatar1.jpg', bio: null },
  { id: 'muted-2', nickname: 'ミュートユーザー2', avatarUrl: '/avatar2.jpg', bio: '盆栽好き' },
]
...
async function makeAuthenticatedRequest(
  ...
  const url = `http://localhost/api/v1/users/me/mutes${searchParams ? `?${searchParams}` : ''}`
...
describe('GET /api/v1/users/me/mutes', () => {
  ...
  it('正常取得で 200 { items, nextCursor } を返す', async () => {
    ...
    const { GET } = await import('@/app/api/v1/users/me/mutes/route')
  ...
  it('getMutedUsersService に userId と limit が渡される', async () => {
    const req = await makeAuthenticatedRequest('user-1', 'user@example.com')
    const { GET } = await import('@/app/api/v1/users/me/mutes/route')
    await GET(req)

    expect(mockGetMutedUsersService).toHaveBeenCalledWith('user-1', expect.any(Number), undefined)
  })
  ...
  it('limit=5 クエリパラメータが反映される', async () => {
    ...
    expect(mockGetMutedUsersService).toHaveBeenCalledWith('user-1', 5, undefined)
  })

  it('cursor クエリパラメータが反映される', async () => {
    const cursorId = 'muted-cursor-id'
    const req = await makeAuthenticatedRequest('user-1', 'user@example.com', `cursor=${cursorId}`)
    const { GET } = await import('@/app/api/v1/users/me/mutes/route')
    await GET(req)

    expect(mockGetMutedUsersService).toHaveBeenCalledWith('user-1', expect.any(Number), cursorId)
  })

  it('limit 件取得時に nextCursor が最後の mutedId になる', async () => {
    mockGetMutedUsersService.mockResolvedValueOnce({
      items: mockMutedUsers,
      nextCursor: 'muted-2',
    })
    ...
    const body = await res.json()
    expect(body.nextCursor).toBe('muted-2')
  })

  it('ミュートがない場合は空配列と nextCursor: null', async () => {
    mockGetMutedUsersService.mockResolvedValueOnce({ items: [], nextCursor: null })
    const req = await makeAuthenticatedRequest('user-1', 'user@example.com')
    const { GET } = await import('@/app/api/v1/users/me/mutes/route')
    const res = await GET(req)
  ...
  it('Bearer ヘッダーなしで 401 AUTH_REQUIRED', async () => {
    const req = new NextRequest('http://localhost/api/v1/users/me/mutes', { method: 'GET' })
    const { GET } = await import('@/app/api/v1/users/me/mutes/route')
    const res = await GET(req)
  ...
  it('不正なトークンで 401 AUTH_INVALID_TOKEN', async () => {
    const req = new NextRequest('http://localhost/api/v1/users/me/mutes', {
