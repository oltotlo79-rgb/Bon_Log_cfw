...
/**
 * GET /api/v1/analytics/likes のユニットテスト
 *
...
const USER_ID = 'user-likes-analytics'
...
const mockFetchLikeAnalytics = vi.fn()
vi.mock('@/lib/services/analytics-service', () => ({
  fetchPostAnalytics: vi.fn(),
  fetchLikeAnalytics: (...args: unknown[]) => mockFetchLikeAnalytics(...args),
...
async function makeBearerRequest(userId: string, searchParams = ''): Promise<NextRequest> {
  ...
  const url = `http://localhost/api/v1/analytics/likes${searchParams ? `?${searchParams}` : ''}`
...
const MOCK_LIKE_ANALYTICS = {
  totalLikes: 120,
  avgLikesPerPost: 6.5,
  mostLikedPostId: 'post-1',
  likesPerDay: [
...
describe('GET /api/v1/analytics/likes', () => {
  beforeEach(() => {
    ...
    mockFetchLikeAnalytics.mockResolvedValue(MOCK_LIKE_ANALYTICS)
  })
  ...
  it('プレミアムユーザーが days=30 で 200 を返す', async () => {
    ...
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    const res = await GET(req)

    expect(res.status).toBe(200)
  })

  it('days 省略時はデフォルト 30 で fetchLikeAnalytics が呼ばれる', async () => {
    const req = await makeBearerRequest(USER_ID)
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    await GET(req)

    expect(mockFetchLikeAnalytics).toHaveBeenCalledWith(USER_ID, 30)
  })

  it('days=7 で fetchLikeAnalytics が 7 で呼ばれる', async () => {
    const req = await makeBearerRequest(USER_ID, 'days=7')
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    await GET(req)

    expect(mockFetchLikeAnalytics).toHaveBeenCalledWith(USER_ID, 7)
  })

  it('days=90 で fetchLikeAnalytics が 90 で呼ばれる', async () => {
    const req = await makeBearerRequest(USER_ID, 'days=90')
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    await GET(req)

    expect(mockFetchLikeAnalytics).toHaveBeenCalledWith(USER_ID, 90)
  })

  it('fetchLikeAnalytics の戻り値をそのまま返す', async () => {
    const req = await makeBearerRequest(USER_ID, 'days=30')
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    const res = await GET(req)
    const body = await res.json()

    expect(body.totalLikes).toBe(MOCK_LIKE_ANALYTICS.totalLikes)
    expect(body.avgLikesPerPost).toBe(MOCK_LIKE_ANALYTICS.avgLikesPerPost)
  })

  it('fetchLikeAnalytics が 1 回だけ呼ばれる', async () => {
    const req = await makeBearerRequest(USER_ID, 'days=30')
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    await GET(req)

    expect(mockFetchLikeAnalytics).toHaveBeenCalledOnce()
  })

  it('Bearer ヘッダーなしで 401 AUTH_REQUIRED', async () => {
    const req = new NextRequest('http://localhost/api/v1/analytics/likes', { method: 'GET' })
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    const res = await GET(req)
  ...
  it('不正なトークンで 401 AUTH_INVALID_TOKEN', async () => {
    const req = new NextRequest('http://localhost/api/v1/analytics/likes', {
      method: 'GET',
  ...
  it('非プレミアム時は fetchLikeAnalytics が呼ばれない', async () => {
    mockIsPremiumUser.mockResolvedValueOnce(false)
    const req = await makeBearerRequest(USER_ID, 'days=30')
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    await GET(req)

    expect(mockFetchLikeAnalytics).not.toHaveBeenCalled()
  })
  ...
  it('レート制限超過時は fetchLikeAnalytics が呼ばれない', async () => {
    mockCheckUserRateLimit.mockResolvedValueOnce({ success: false, remaining: 0, resetTime: Date.now() + 30000 })
    const req = await makeBearerRequest(USER_ID, 'days=30')
    const { GET } = await import('@/app/api/v1/analytics/likes/route')
    await GET(req)
