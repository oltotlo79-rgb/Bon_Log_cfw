...
function createAvatarReq(type = 'image/jpeg', size = 1000, name = 'avatar.jpg') {
...
describe('Upload Avatar API', () => {
  ...
  it('delegates to handleProfileImageUpload with "avatar" type (returns upload result)', async () => {
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://example.com/avatar.jpg' })
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq() as unknown as NextRequest)
    const data = await res.json()
    expect(res.status).toBe(200)
    expect(data.url).toBe('https://example.com/avatar.jpg')
  })
  ...
  it('returns 400 if no file', async () => {
    const fd = new FormData()
    const req = new Request('http://localhost/api/upload/avatar', { method: 'POST', body: fd })
    const { POST } = await import('@/app/api/upload/avatar/route')
  ...
  it('returns 400 for oversized file', async () => {
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq('image/jpeg', 5 * 1024 * 1024) as unknown as NextRequest)
    expect(res.status).toBe(400)
  })

  it('returns 400 for invalid type', async () => {
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq('image/gif', 1000, 'avatar.gif') as unknown as NextRequest)
    expect(res.status).toBe(400)
  })
  ...
  it('uploads successfully', async () => {
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://cdn/avatar.jpg' })
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq() as unknown as NextRequest)
    const data = await res.json()
    expect(res.status).toBe(200)
    expect(data.url).toBe('https://cdn/avatar.jpg')
  })

  it('deletes old avatar on success', async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ avatarUrl: 'https://cdn/old.jpg', headerUrl: null })
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://cdn/new.jpg' })
    mockDeleteFile.mockResolvedValue(undefined)
    const { POST } = await import('@/app/api/upload/avatar/route')
    await POST(createAvatarReq() as unknown as NextRequest)
    expect(mockDeleteFile).toHaveBeenCalledWith('https://cdn/old.jpg')
  })

  it('does not delete placeholder avatar', async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ avatarUrl: '/placeholder.png', headerUrl: null })
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://cdn/new.jpg' })
    const { POST } = await import('@/app/api/upload/avatar/route')
    await POST(createAvatarReq() as unknown as NextRequest)
  ...
  it('accepts PNG', async () => {
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://cdn/a.png' })
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq('image/png', 1000, 'a.png') as unknown as NextRequest)
    expect(res.status).toBe(200)
  })

  it('accepts WebP', async () => {
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://cdn/a.webp' })
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq('image/webp', 1000, 'a.webp') as unknown as NextRequest)
    expect(res.status).toBe(200)
  })
  ...
  it('returns 400 if image-file signature validation fails', async () => {
    const { validateImageFile } = await import('@/lib/file-validation')
    vi.mocked(validateImageFile).mockReturnValueOnce({ valid: false, error: 'invalid signature' } as ReturnType<typeof validateImageFile>)
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq() as unknown as NextRequest)
    expect(res.status).toBe(400)
    const data = await res.json()
  ...
  it('uses ERR_INVALID_IMAGE_FORMAT fallback if validateImageFile returns no error string', async () => {
    const { validateImageFile } = await import('@/lib/file-validation')
    vi.mocked(validateImageFile).mockReturnValueOnce({ valid: false } as ReturnType<typeof validateImageFile>)
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq() as unknown as NextRequest)
    expect(res.status).toBe(400)
    const data = await res.json()
    // ERR_INVALID_IMAGE_FORMAT が使われる
    expect(data.error).toContain('JPEG')
  })

  it('returns 500 with ERR_UPLOAD_SERVER_ERROR when an unexpected exception occurs', async () => {
    mockPrisma.user.findUnique.mockRejectedValueOnce(new Error('DB connection error'))
    mockUploadFile.mockResolvedValue({ success: true, url: 'https://cdn/a.jpg' })
    const { POST } = await import('@/app/api/upload/avatar/route')
    const res = await POST(createAvatarReq() as unknown as NextRequest)
    expect(res.status).toBe(500)
    const data = await res.json()
