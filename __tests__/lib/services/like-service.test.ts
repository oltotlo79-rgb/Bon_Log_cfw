...
describe('togglePostLikePrimitive', () => {
  ...
  it('未いいねの場合 liked: true を返す', async () => {
    mockPrisma.$transaction.mockImplementationOnce(async () => true)
  ...
  it('いいね済みの場合 liked: false を返す（解除）', async () => {
    mockPrisma.$transaction.mockImplementationOnce(async () => false)
    const { togglePostLikePrimitive } = await import('@/lib/services/like-service')
    const result = await togglePostLikePrimitive('post-1', 'user-1')
