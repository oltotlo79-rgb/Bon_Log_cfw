/**
 * @fileoverview プロフィール編集ページのローディング画面
 *
 * このファイルはNext.js App Routerの規約に従ったローディングUIコンポーネントです。
 * /settings/profileページのデータ取得中に表示されるフォールバックUIを提供します。
 *
 * Next.jsの仕組み:
 * - loading.tsxファイルは自動的にSuspenseバウンダリとして機能します
 * - ページコンポーネント（page.tsx）のデータフェッチ中に自動表示されます
 * - ユーザーのプロフィール情報取得完了後は自動的にページコンテンツに切り替わります
 *
 * なぜ必要か:
 * - ユーザーにプロフィール情報読み込み中であることを伝える（UX向上）
 * - データベースからのプロフィールデータ取得に時間がかかる場合の待機状態を表現
 * - ページ遷移がスムーズに見えるようにする
 *
