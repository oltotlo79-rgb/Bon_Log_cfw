...
import { getFertilizerColumnBySlug } from '@/lib/actions/fertilizer'
...
import { BASE_URL, ROUTE_FERTILIZERS_COLUMNS } from '@/lib/constants/routes'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const col = await getFertilizerColumnBySlug(slug)
  if (!col) return { title: 'コラムが見つかりません - BON-LOG' }
  return {
    title: `${col.title} - コラム - BON-LOG`,
    alternates: { canonical: pageCanonical(`${ROUTE_FERTILIZERS_COLUMNS}/${slug}`) },
  }
}

export async function generateStaticParams() {
  return loadStaticParams(async () => {
    const items = await prisma.fertilizerColumn.findMany({ select: { slug: true } })
    return items.map((i) => ({ slug: i.slug }))
  }, '/fertilizers/columns')
}

export default async function FertilizerColumnDetailPage({ params }: Props) {
  const { slug } = await params
  const col = await getFertilizerColumnBySlug(slug)
  ...
  return (
    <div className="space-y-6">
      <ArticleJsonLd
        ...
        url={`${BASE_URL}${ROUTE_FERTILIZERS_COLUMNS}/${slug}`}
        description={col.content?.slice(0, COLUMN_OG_DESCRIPTION_LENGTH) || col.title}
      />

      <Link href={ROUTE_FERTILIZERS_COLUMNS} className="inline-flex items-center gap-1 text-sm text-primary hover:underline">
