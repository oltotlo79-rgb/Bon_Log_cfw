/**
 * @module app/api/v1/users/me/mutes
...
import { getMutedUsersService } from '@/lib/services/mute-service'
...
export async function GET(request: NextRequest) {
  ...
  // 4. ミュート一覧取得
  const mutedUsersResult = await getMutedUsersService(auth.userId, limit, cursor)
  ...
  return NextResponse.json({
    items: mutedUsersResult.items,
    nextCursor: mutedUsersResult.nextCursor,
  })
}
