/**
 * @module app/api/v1/users/[id]/following
 * GET /api/v1/users/{id}/following — ユーザーのフォロー中一覧取得
 */
...
import { fetchUserFollowing } from '@/lib/services/user-connections-service'
import {
  resolveFollowStates,
...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  // 4. フォロー中一覧取得（可視性チェック込み）
  const result = await fetchUserFollowing(
    targetUserId,
    auth.userId,
    parsed.data.cursor,
    parsed.data.limit,
