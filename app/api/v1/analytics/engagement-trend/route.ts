/**
 * @module app/api/v1/analytics/engagement-trend
 * GET /api/v1/analytics/engagement-trend — 期間内の日次エンゲージメント推移を取得する
 *
...
import { fetchEngagementTrend } from '@/lib/services/analytics-service'
...
export async function GET(request: NextRequest) {
  ...
  // 6. 集計クエリ実行（date は isoDateKey で文字列済みのため変換不要）
  const data = await fetchEngagementTrend(auth.userId, daysNum)
