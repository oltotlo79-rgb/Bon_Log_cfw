/**
 * @module app/api/v1/analytics/genre-performance
 * GET /api/v1/analytics/genre-performance — 期間内のジャンル別パフォーマンスを取得する
 *
...
import { fetchGenrePerformance } from '@/lib/services/analytics-service'
...
export async function GET(request: NextRequest) {
  ...
  // 6. 集計クエリ実行（戻り値はすべてプリミティブのため変換不要）
  const data = await fetchGenrePerformance(auth.userId, daysNum)
